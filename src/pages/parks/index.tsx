...
import { ParkInterface } from 'interfaces/park';
import { useRoqClient, useParkFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<ParkInterface, unknown>;

interface ParkListPageProps {
...
export function ParkListPage(props: ParkListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useParkFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.park.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: ParkInterface) => {
    if (hasAccess('park', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/parks/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/parks/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('park', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/parks/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('park', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
